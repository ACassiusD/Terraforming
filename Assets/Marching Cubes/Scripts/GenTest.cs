...
public class GenTest : MonoBehaviour
{
    ...
    public int chunksPerAxis = 10; //Cube this number to get the total # Of chunks we split up the terrain boundary into. (i.e. 10^3=100 chunks total)
	...
	//Buffers
	ComputeBuffer triangleBuffer; //Stores the vertices generated by the Marching Cubes algorithm, an array of VertexData structs
    ComputeBuffer triCountBuffer; //Stores the number of triangles generated by the Marching Cubes algorithm

    [HideInInspector] public RenderTexture rawDensityTexture;
	[HideInInspector] public RenderTexture processedDensityTexture;

    //Chunks. Small manageable pieces of the terrain.
	//contain coordinates, center, size, and a GameObject for mesh rendering.
    Chunk[] chunkArray;
	...
	void Start()
	{
        //TODO: Update CreateChunks and try and make it not a spehere/cube but more rectangular
	...
	void InitTextures()
	{
        ...
        int terrainBoundingBoxAxisLength = chunksPerAxis * (numPointsPerAxis - 1) + 1;
    ...
    void CreateChunks()
    {
        chunkArray = new Chunk[chunksPerAxis * chunksPerAxis * chunksPerAxis];

        float chunkSize = (terrainBoundarySize) / chunksPerAxis;
        int i = 0;
        ...
        for (int y = 0; y < chunksPerAxis; y++)
        {
            for (int x = 0; x < chunksPerAxis; x++)
            {
                for (int z = 0; z < chunksPerAxis; z++)
                {
                    ...
                    float posX = (-(chunksPerAxis - 1f) / 2 + x) * chunkSize;
                    float posY = (-(chunksPerAxis - 1f) / 2 + y) * chunkSize;
                    float posZ = (-(chunksPerAxis - 1f) / 2 + z) * chunkSize;
    ...
    //First use the density compute shader to generate the density values for each point in the 3D terrain texture
